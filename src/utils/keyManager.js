...
function loadInvalidCookiesFromFile() {
  ...
  try {
    if (fs.existsSync(INVALID_COOKIES_FILE)) {
      // 直接从磁盘读取文件，不使用缓存
      const data = fs.readFileSync(INVALID_COOKIES_FILE, { encoding: 'utf8', flag: 'r' });
...
function loadApiKeysFromFile() {
  ...
  try {
    if (fs.existsSync(API_KEYS_FILE)) {
      // 直接从磁盘读取文件，不使用缓存
      const data = fs.readFileSync(API_KEYS_FILE, { encoding: 'utf8', flag: 'r' });
